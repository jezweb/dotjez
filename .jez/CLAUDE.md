...
One topic per file, named in **kebab-case** after the thing it's about (`acme-corp.md`, not `Acme Corp.md` or `acme (old).md`). The first two lines should tell a reader whether it's relevant. Lead with the point, then back it up. **To add a new one, copy the example already in that folder** — the examples are the templates. (A few shipped files are real starter content rather than samples; the samples say so at the top. Anything without that note is yours to keep, update, or bin like any other note.)
...
created: 2026-06-09
updated: 2026-06-09
status: current   # the staples: current | needs-review | archived. Others earn their place where they fit (active, promoted, placeholder)
---
```
