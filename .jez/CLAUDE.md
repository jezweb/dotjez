...
One topic per file. The first two lines should tell a reader whether it's relevant. Lead with the point, then back it up. **To add a new one, copy the example already in that folder** — the examples are the templates.

Frontmatter is optional. It earns its place on `clients/` and `projects/` (a status or date you'll filter on); playbooks and research often have none. Don't fill in a form for its own sake.
...
- **Keep the briefs current.** When you change how things are organised or settle a new convention, update this file. Give a folder its own short `CLAUDE.md` once it grows its own rules (a naming scheme, a sub-structure, a collection-specific gotcha); until then the example file is enough. Stale instructions are worse than none.
...
- **Name files well** and you won't need index files: `ls` + grep + `[[links]]` is enough.
