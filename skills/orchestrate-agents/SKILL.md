...
- **Fence it, don't script it.** The guardrails that make a model-driven loop safe to trust: a step cap per turn, a recovery / no-progress budget, tools scoped per turn to what this step needs, an approval gate on destructive or outward actions, a tight task spec per run rather than open-ended chat, and shadow mode until the output is verified by inspection. Those bound the loop's freedom without pre-deciding it. (A managed harness — Claude Code as a library, the Agents SDK Think class — usually provides these as primitives; reach for it before hand-rolling durability, polling, and context-packing.)
