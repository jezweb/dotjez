...
- **Stance.** "You are a senior engineer" adds nothing; *whose eyes to look through and what failure costs* adds a lot ("you're reviewing this as the person on call for it", "write for a non-technical client nervous about cost"). Identity, lane, audience, and stakes steer the output; capability adjectives never do. The sharpest stake is what the output *gates*: "your confidence rating decides whether this is persisted" changes the rating; the rule alone gets followed nominally.
...
**Derived facts: right or null, never confidently wrong.** When the model computes, measures, or infers a value rather than copying it from the source, design for abstention: emit the value only when the source explicitly supports it, otherwise leave it null for a human. A null is recoverable; a confident wrong value gets acted on. Have the model return the parts it can read and let *code* do the arithmetic — and the serialisation: when output must be structurally exact (YAML, frontmatter, timestamps, key names), the model decides the *values* and code owns the *format*, and a whole class of malformed-output bugs disappears at once. This splits the deterministic from the judgment — code owns the parts with one right answer it computes exactly (format, ids, dedup, arithmetic), the model owns every value and every judgment call. It is not a preference for code over the model, and the split is not fixed: where you draw it is dated evidence about what today's models get wrong, so it migrates toward the model as they improve (`plan-and-build` covers that moving boundary). Don't trust self-reported confidence, and don't trust one happy-path sample: test across the real distribution and measure the confidently-wrong rate, because that's the number that matters.
...
**Let "nothing" be an answer.** A required slot gets filled: "end with a summary of flags" manufactures flags when there are none, because the model satisfies the shape under completion pressure. When absence is legitimate — no findings, no issues, nothing to escalate — permit it explicitly ("no findings is a valid result") and make the slot optional. The open-ended sibling of right-or-null.
...
**Another model can advise on a prompt; don't let it iterate on one.** A single-turn interview for structural gaps ("what does your family misread in this input? what about the setup matters?") surfaces failure modes no docs carry. But its advice is a menu, not a diff: adopt selectively and A/B each piece, because stacking every offered guardrail over-specifies the prompt until legitimate output gets suppressed. Refinement-by-discussion on a working prompt is worse: each turn harvests over-specification, and a refiner shown your failing cases will quietly bake them into the prompt as worked examples and hand back a perfect score on the inputs it memorised — score any refined prompt only on inputs the refiner never saw.
