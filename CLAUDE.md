...
Keep it private: this workspace will hold real people and client data, so it lives in a private repo or local-only, never a public remote. Don't suggest publishing it. And the converse, when you contribute an improvement *out* to a public repo from work on a real project: strip the client and project identity first (names, hostnames, account IDs, secrets, real file paths) and contribute the generalisable pattern, not the identifying artifact. You're swimming in that project's context, so the leak is easy to miss; public is effectively forever, so the check happens before the commit, not after.
