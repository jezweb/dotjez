...
Keep it private: this workspace will hold real people and client data, so it lives in a private repo or local-only, never a public remote. Don't suggest publishing it. And don't let private detail flow the other way either: when contributing to any public repo, keep client and project specifics out of it — share the general pattern, not the private information.
